defmodule ShoeShopWeb.LiveShoeShopTest do
  ...
  describe "handle_event/3" do
    ...
    test "changeShoeTypeSelection event: when selected type is men and change type is women, updates assigns and returns socket", %{socket: socket} do
    ...
    test "changeShoeTypeSelection event: when selected type is women and change type is men, updates assigns and returns socket", %{socket: socket} do
    ...
    test "changeShoeTypeSelection event: when selected type matches the change type, returns socket only", %{socket: socket} do
      {:ok, socket} = LiveShoeShop.mount(%{}, %{}, socket)
      {:noreply, socket_2} = LiveShoeShop.handle_event("changeShoeTypeSelection", %{"value" => "Men"}, socket)

      assert socket.assigns == socket_2.assigns
    end

    test "renderShoePreview event: when a shoe id is provided, updates assigns and returns socket", %{socket: socket} do
      {:ok, socket} = LiveShoeShop.mount(%{}, %{}, socket)
    ...
    test "addToCart event: when id and size are provided, updates assigns and returns socket", %{socket: socket} do
      ...
      } = LiveShoeShop.handle_event("addToCart", %{"id" => "add-to-cart-shoe-id-#{shoe.id}", "size" => 7}, socket)
    ...
    test "addToCart event: when a cart exists and size/id are provided, updates assigns and returns socket", %{socket: socket} do
      ...
      } = LiveShoeShop.handle_event("addToCart", %{"id" => "add-to-cart-shoe-id-#{shoe_2.id}", "size" => 6}, socket)
    ...
    test "removeCartItem event: when id is provided, removes correct id from cart, updates assigns, and returns socket", %{socket: socket} do

      {:ok, socket} = LiveShoeShop.mount(%{}, %{}, socket)
      shoe = Enum.at(socket.assigns.shoes, 0)

      {:noreply, socket} = LiveShoeShop.handle_event("addToCart", %{"id" => "add-to-cart-shoe-id-#{shoe.id}", "size" => 7}, socket)

      {:noreply,
        %Phoenix.LiveView.Socket{
          assigns: assigns
        } = _socket
      } = LiveShoeShop.handle_event("removeCartItem", %{"id" => shoe.id}, socket)

      assert [] == assigns.cart
    end

    test "removeCartItem event: when id is provided and multiple items exist in cart, removes correct id from cart, updates assigns, and returns socket", %{socket: socket} do
      {:ok, socket} = LiveShoeShop.mount(%{}, %{}, socket)
      shoe = Enum.at(socket.assigns.shoes, 0)
      shoe_2 = Enum.at(socket.assigns.shoes, 1)
      ...
      {:noreply, socket} = LiveShoeShop.handle_event("addToCart", %{"id" => "add-to-cart-shoe-id-#{shoe_2.id}", "size" => 6}, socket)
    ...
    test "showCheckoutModal event: updates assigns and returns socket", %{socket: socket} do
      ...
      } = LiveShoeShop.handle_event("showCheckoutModal", %{}, socket)

      assert true == assigns.show_checkout
    end

    test "closeModal event: given a modal value of preview, updates assigns and returns socket", %{socket: socket} do
      {:ok, socket} = LiveShoeShop.mount(%{}, %{}, socket)
      shoe = Enum.at(socket.assigns.shoes, 0)

      {:noreply, socket} = LiveShoeShop.handle_event("renderShoePreview", %{"shoe_id" => shoe.id}, socket)
      ...
      } = LiveShoeShop.handle_event("closeModal", %{"modal" => "preview"}, socket)

      assert nil == assigns.preview


    end
    test "closeModal event: given a modal value of checkout, updates assigns and returns socket", %{socket: socket} do
      {:ok, socket} = LiveShoeShop.mount(%{}, %{}, socket)
      ...
      } = LiveShoeShop.handle_event("closeModal", %{"modal" => "checkout"}, socket)
    ...
    test "closeModal event: given any other modal value, just returns socket", %{socket: socket} do
      {:ok, socket} = LiveShoeShop.mount(%{}, %{}, socket)
      {:noreply, socket} = LiveShoeShop.handle_event("showCheckoutModal", %{}, socket)
      {:noreply, socket_2} = LiveShoeShop.handle_event("closeModal", %{"modal" => "a-value"}, socket)
